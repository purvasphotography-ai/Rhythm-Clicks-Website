document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ==========================================================================
     9. Janmashtami Promotion Popup Logic
     ========================================================================== */
  const promoPopup = document.getElementById('janmashtami-promo-popup');
  const promoClose = document.getElementById('janmashtami-popup-close');
  const promoSkip = document.getElementById('janmashtami-popup-skip');
  ...
  if (promoPopup) {
    ...
    const isShown = sessionStorage.getItem('janmashtamiPopupShown');
    
    if (!isShown) {
      // Show popup after a 2-second delay for premium, relaxed experience
      setTimeout(() => {
        promoPopup.classList.add('active');
        document.body.style.overflow = 'hidden';
        sessionStorage.setItem('janmashtamiPopupShown', 'true');
      }, 2000);
    }
    ...
    // Close on Skip button click
    if (promoSkip) {
      promoSkip.addEventListener('click', closePromoPopup);
    }

    // Close on clicking backdrop overlay directly
